...
{
 	printf("◆ 말타고 화살쏘기 ◆\n\n");
  	printf("말을 타고 목표물(■)을 맞추는 게임입니다.\n");
 	printf("화살은 스페이스키로 발사합니다. \n\n");
 	printf("- 도전자 이름을 입력하고 ENTER > ");
	scanf("%s", name[0]);
	printf("\n- 게임 진행 횟수를 입력하고 ENTER > ");
	scanf("%d", n);
	printf("\n- 게임 난이도(1~3) 입력하고 ENTER > ");
	scanf("%d", level);
	while(*level < 1 || *level > 3)
	{
		printf("올바른 난이도를 입력해주세요(1~3) > ");
		scanf("%d", level);
	}
	if(*level == 1) *level = 40;
	...
	{
		printf("올바른 번호를 입력해주세요(1=▲|2=♣|3=ψ|4=∧) > ");
		scanf("%d", horseno);
	}
}
